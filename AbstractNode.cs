...
namespace jsxbin_to_jsx.JsxbinDecoding
{
    public abstract class AbstractNode : INode
    {
        ...
        {
			string normalized = jsxbin.Replace (System.Environment.NewLine, "").Replace ("\r", "");//.Replace("\\", "");
		...
		// Special cases need to be made here for certain escaped strings.
        public string DecodeVariantCore()
        {
            ...
            switch (typ)
            {
                ...
                case 3:	// number
                    val = DecodeNumber();
                    break;
                case 4: // ***  This only affects the output strings
                    val = DecodeString();
                    val = val.Replace("\\", "\\\\");// change double \\ to escaped \\
					val = val.Replace("\'", "\\\'"); // not invoked
                    val = "\'" + val + "\'"; // put close ' on statements
					//val = val.Replace("\'","\'-yo"); // open and close '
					val = val.Replace("\n", "\\n");
